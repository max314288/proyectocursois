-- ============================================================
--  EJEMPLO COMPLETO — Menú v5 (SQL Server)
--  Restaurante: "La Trattoria di Luigi"
--  Con transacción y rollback ante cualquier error
-- ============================================================
...
BEGIN TRY
    ...
    -- Scratch para OUTPUTs que no usamos
    ...
    IF @rest_id IS NULL
        THROW 60001, 'El restaurante "La Trattoria di Luigi" no existe. Ejecuta primero el script de data base.', 1;

    -- Validar que no haya items previos (evitar duplicados)
    IF EXISTS (SELECT 1 FROM items_menu WHERE restaurante_id = @rest_id)
        THROW 60002, 'El restaurante ya tiene items registrados. Limpia primero los datos para evitar duplicados.', 1;

    PRINT '→ Iniciando carga del menú...';
    ...
    PRINT '  ✓ Platos a la carta creados (5)';
    ...
    PRINT '  ✓ Menús compuestos creados (3)';
    ...
    PRINT '  ✓ Arma tu plato creado (1 con 11 opciones)';

    COMMIT TRANSACTION;

    PRINT '';
    PRINT '════════════════════════════════════════════════';
    PRINT '✓ Menú v5 insertado correctamente';
    PRINT '════════════════════════════════════════════════';
...
BEGIN CATCH
    ...
    DECLARE @error_lin INT             = ERROR_LINE();
    DECLARE @error_proc NVARCHAR(200)  = ISNULL(ERROR_PROCEDURE(), 'script');

    PRINT '';
    PRINT '════════════════════════════════════════════════';
    PRINT '✗ ERROR — Se hizo ROLLBACK de toda la transacción';
    PRINT '════════════════════════════════════════════════';
    PRINT 'Número  : ' + CAST(@error_num AS VARCHAR);
    PRINT 'Línea   : ' + CAST(@error_lin AS VARCHAR);
    PRINT 'Origen  : ' + @error_proc;
    PRINT 'Mensaje : ' + @error_msg;
    PRINT '════════════════════════════════════════════════';
